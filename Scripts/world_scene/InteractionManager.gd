...
## TerrainObjectManager 引用
var _terrain_object_manager: Node = null
...
func _cache_references() -> void:
	...
	_terrain_object_manager = get_node_or_null("/root/World/Managers/TerrainObjectManager")
	if not _terrain_object_manager:
		# Fallback attempt if not under World/Managers
		_terrain_object_manager = get_tree().current_scene.find_child("TerrainObjectManager", true, false)
	
	if not _terrain_object_manager:
		push_warning("InteractionManager: TerrainObjectManager not found")
		...
		if selected_unit:
			...
			if _is_within_interaction_range(selected_unit, global_pos):
				...
				if selected_unit.has_method("command_interact"):
					# 尝试与地形物体交互
					if _terrain_object_manager:
						var tile_coord := _MapUtils.world_to_tile(global_pos)
						# 请求交互 (传入 -1 表示自动扫描所有层级，优先上层)
						var entity = _terrain_object_manager.request_interaction(tile_coord, -1)
							
						if entity:
							selected_unit.command_interact(entity)
							return
					
					# 如果不是地形物体，则传递空或者位置 (取决于 command_interact 实现)
