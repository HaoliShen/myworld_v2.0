...
func can_handle(target: Node) -> bool:
	...
	if comp and comp.can_accept_interaction(&"attack"):
		return true
	return false
...
func execute(target: Node) -> void:
	...
	# 检查距离 (可以依赖 InteractableComponent 的 check，也可以双重检查)
	# var target_pos = component.interaction_area.global_position
	# if not _is_in_range(target_pos): ...

	print("Attacking: ", comp.owner_node.name if comp.owner_node else comp.get_parent().name)
